...
class OnOffSVGPBatch(BayesianModel, ExternalDataTrainingLossMixin):
    """
    ...
    def __init__(self, num_data, num_latent, kernf, kerng, likelihood, Zf, Zg, mean_function=None, minibatch_size=None,
                 name='model'):
        ...
        self.num_latent = num_latent  # num_latent will be 1
    ...
    def build_likelihood(self, data):
        ...
        var_exp = self.likelihood.variational_expectations(gfmean, gfvar, gfmeanu, Y)

        # re-scale for minibatch size
        scale = tf.cast(self.num_data, default_float()) / \
                tf.cast(tf.shape(X)[0], default_float())

        return tf.reduce_sum(var_exp) * scale - KL

    def maximum_log_likelihood_objective(self, data):
        """We require this function"""
        return self.build_likelihood(data)
